...
const fetchData = async () => {
  ...
  try {
    ...
    // 2. Fetch Current Episode & All Episodes (Parallel)
    const [epRes, allEpsRes] = await Promise.all([
      supabase.from('episodes').select('*').eq('anime_id', animeData.id).eq('episode_number', ep).single(),
      supabase.from('episodes').select('*').eq('anime_id', animeData.id).order('episode_number', { ascending: true })
    ])
    
    if (epRes.error || !epRes.data) throw new Error('Episode not found')
    episode.value = epRes.data as Episode
    episodes.value = (allEpsRes.data || []) as Episode[]
