    ...
    <div class="container mx-auto px-6 max-w-5xl space-y-12">
      ...
      <div class="relative animate-reveal-up">
        ...
        <div class="px-12 -mt-20 flex flex-col md:flex-row items-end gap-8 relative z-10">
          <div class="relative group">
            <div class="w-40 h-40 rounded-[2.5rem] bg-surface-gox border-4 border-background overflow-hidden shadow-2xl">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 animate-reveal-up" style="animation-delay: 0.1s">
        <div v-for="stat in stats" :key="stat.label" class="glass-panel p-8 rounded-[2.5rem] border border-border-gox flex items-center justify-between">
           ...
           <div class="w-12 h-12 rounded-2xl bg-surface-gox flex items-center justify-center">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 animate-reveal-up" style="animation-delay: 0.2s">
        ...
        <div class="space-y-4">
           ...
           <button class="w-full flex items-center gap-4 p-5 rounded-2xl bg-surface-gox border border-primary/30 text-primary shadow-xl shadow-primary/5 transition-all">
             <User class="w-5 h-5" />
             <span class="text-sm font-black uppercase tracking-widest">Personal Info</span>
             <Check class="w-4 h-4 ml-auto" />
           </button>
           <button class="w-full flex items-center gap-4 p-5 rounded-2xl bg-transparent border border-border-gox text-muted hover:border-primary/20 hover:text-foreground transition-all">
        ...
        <div class="lg:col-span-2 space-y-8">
           <div class="glass-panel p-10 rounded-[3rem] border border-border-gox space-y-8">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div class="space-y-2">
                   ...
                   <input type="text" :value="user.username" disabled class="w-full bg-surface-gox border border-border-gox rounded-xl p-4 text-sm font-bold opacity-60" />
                 </div>
                 <div class="space-y-2">
                   <label class="text-[10px] font-black uppercase tracking-widest text-muted">Email Address</label>
                   <input type="email" :value="user.email" disabled class="w-full bg-surface-gox border border-border-gox rounded-xl p-4 text-sm font-bold opacity-60" />
                 </div>
                 <div class="md:col-span-2 space-y-2">
                   <label class="text-[10px] font-black uppercase tracking-widest text-muted">Bio / Status</label>
                   <textarea placeholder="Write something about yourself..." class="w-full bg-surface-gox border border-border-gox rounded-xl p-4 text-sm focus:outline-none focus:border-primary transition-all resize-none h-32"></textarea>
                 </div>
              </div>

              <div class="pt-6 border-t border-border-gox flex justify-end">
